/*
 * Haushaltsplaner Developer Beta 0.81
 *
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV081', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    localStorage.setItem('budgetStateV081', JSON.stringify(state));
  }
...
function showPersonalEditor(personId, editPost) {
  ...
  function renderDebts() {
    ...
    const visibleDebts = state.debts.filter((d) => shouldShowDebtInMonth(d, currentMonth));
    ...
    if (visibleDebts.length === 0) {
      ...
      visibleDebts.forEach((d) => {
        ...
        else if (!(Number(d.monthlyRate || 0) > 0) && Number(d.amountOpen || 0) > 0) statusHtml = '<span class="pill warning">Ratenplan offen</span>';
      ...
      },
      {
        ...
        onClick: (close) => {
          ...
          if (editDebt) {
            ...
            const newDebt = { id: generateId(), name, amountOpen: open, monthlyRate: rate, nextDueMonth: due, paidMonths: [] };
...
function renderPots() {
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.81 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.81 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.81 geladen';
    }
