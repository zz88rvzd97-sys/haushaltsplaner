/*
 * Haushaltsplaner Developer Beta 0.86
 *
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV086', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    try {
      ...
      localStorage.setItem('budgetStateV086', payload);
  ...
  // und die persönlichen Ausgaben berücksichtigt. historische Puffer-Ausgaben werden nicht mehr in die Verteilung eingerechnet.
...
2 = ab diesem Monat dauerhaft`,
  ...
  function renderOverview() {
    ...
    const totalAvail = totalAvailBeforeBuffer;
  ...
  function renderBufferExpenses() {
    ...
    card.appendChild(createSummaryMetrics([
      ...
      { label: 'Rest', value: `${remaining.toFixed(2)} €`, kind: remaining > 0 ? 'success' : 'warning' }
    ]));

    const hint = document.createElement('p');
    hint.className = 'small muted';
    hint.textContent = 'Hier trägst du Ausgaben ein, die vom 150€-Puffer bezahlt werden. Nur der Status „bezahlt“ reduziert den Rest. Rücklagen & Sparen bleiben davon unberührt.';
...
function showPersonalEditor(personId, editPost) {
  ...
  function showBufferExpenseEditor(editPost) {
    ...
    const info = document.createElement('p');
    info.className = 'small muted';
    info.textContent = `Diese Ausgaben werden im Puffer vorgemerkt. Erst wenn du sie als bezahlt markierst, reduzieren sie den Rest für ${formatMonthLabel(currentMonth)}. Rücklagen & Sparen werden dadurch nicht reduziert.`;
    content.appendChild(info);
...
function renderSavings() {
    ...
    if (beforeStart) {
      ...
      info.innerHTML = `Am Monatsende verteilst du den verfügbaren Restbetrag in Rücklagen und Sparen. Es wird kein Puffer mehr vorher abgezogen.`;
    }
    card.appendChild(info);
...
function renderPots() {
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.86 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.86 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.86 geladen';
    }
