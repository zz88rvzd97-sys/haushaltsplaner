/*
 * Haushaltsplaner Developer Beta 0.93
 *
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV093', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    try {
      ...
      localStorage.setItem('budgetStateV093', payload);
  ...
  function buildSnowballPlan(startMonth = currentMonth, maxMonths = 72) {
    ...
    const noRate = sourceDebts.filter((debt) => debt.rate <= 0);
    const active = sourceDebts.filter((debt) => debt.rate > 0);
    const rows = [];
    const events = [];
    let rollover = 0;
...
function showPersonalEditor(personId, editPost) {
  ...
  function renderDebts() {
    ...
    const estimatedDebtFree = state.debts
      .filter((d) => Number(d.amountOpen || 0) > 0 && Number(d.monthlyRate || 0) > 0)
    ...
    if (visibleDebts.length === 0) {
      ...
      table.className = 'list-table';
      const thead = document.createElement('thead');
      thead.innerHTML = `<tr><th>Name</th><th>Offen</th><th>Monatsrate</th><th>Nächste Fälligkeit</th><th>Vorauss. Ende</th><th>Fortschritt</th><th>Status</th><th>Bezahlt?</th><th>Aktion</th></tr>`;
      table.appendChild(thead);
      ...
      visibleDebts.forEach((d) => {
        ...
        tr.innerHTML = `<td>${d.name}</td><td>${Number(d.amountOpen || 0).toFixed(2)} €</td><td>${Number(d.monthlyRate || 0).toFixed(2)} €</td><td>${d.nextDueMonth || '-'}</td><td>${estimatedEnd || '-'}</td><td>${progressHtml}</td><td>${statusHtml}</td><td></td><td></td>`;
  ...
  function renderSnowballPlanCard(monthKey) {
    ...
    const p = document.createElement('p');
    p.className = 'small muted';
    p.textContent = 'Wenn eine Schuld z. B. im Juni abbezahlt ist, wird ihre normale Rate erst ab Juli auf die dann kleinste offene Schuld umgelegt.';
    card.appendChild(p);
    if (plan.noRate.length > 0) {
      const warn = document.createElement('div');
      warn.className = 'notice warning';
      warn.textContent = `Nicht im Schneeball enthalten, weil keine Monatsrate hinterlegt ist: ${plan.noRate.map((d) => d.name).join(', ')}`;
      card.appendChild(warn);
    }
...
function renderPots() {
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.93 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.93 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.93 geladen';
    }
