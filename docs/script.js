/*
 * Haushaltsplaner Developer Beta 0.89
 *
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV089', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    try {
      ...
      localStorage.setItem('budgetStateV089', payload);
...
function renderPots() {
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.89 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.89 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.89 geladen';
    }
