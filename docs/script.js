/*
 * Haushaltsplaner Developer Beta 0.87
 *
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV087', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    try {
      ...
      localStorage.setItem('budgetStateV087', payload);
...
function renderPots() {
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.87 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.87 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.87 geladen';
    }
