/*
 * Haushaltsplaner Developer Beta 0.79
 *
...
(() => {
  ...
  function syncDebtPaymentFromPost(post, monthKey) {
    ...
    return addDebtPayment(debt, {
      month: monthKey,
      amount: Number(debt.monthlyRate || getEffectiveAmountForMonth(post, monthKey) || 0),
  ...
  let state;
  try {
  let saved = localStorage.getItem('budgetStateV079');
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV079', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    localStorage.setItem('budgetStateV079', JSON.stringify(state));
  }
  ...
  // Monats‑Key vor Nutzung des Mindestpuffers. Dabei werden die
  // Nettoeinkommen, die gerundeten Anteile der gemeinsamen Kosten
  // und die persönlichen Ausgaben berücksichtigt. Puffer-Ausgaben
  // werden bewusst NICHT abgezogen, damit Rücklagen & Sparen nicht
  // sinken, wenn der Mindestpuffer genutzt wird.
...
2 = ab diesem Monat dauerhaft`,
  ...
  // ----- Rendering -----
  function render() {
    syncCurrentMonthToActualDate();
    renderOverview();
    renderCommon();
    renderPersonal();
    renderBufferExpenses();
    renderTankCalc();
    renderDebts();
    renderSettings();
    renderSavings();
  ...
  function getDataCheckItems() {
    ...
    allPosts.forEach(({ area, post }) => {
      ...
      if (linked && isDue(post, currentMonth)) {
        ...
        const debtRate = Number(linked.monthlyRate || 0);
    ...
    (state.debts || []).forEach((debt) => {
      ...
      if (Number(debt.amountOpen || 0) > 0 && !(Number(debt.monthlyRate || 0) > 0)) {
        items.push({
          kind: 'warning',
          area: 'Schulden',
      ...
      if (debt.nextDueMonth === currentMonth && Number(debt.monthlyRate || 0) > 0) {
  ...
  function renderOverview() {
    ...
    monthRow.className = 'row overview-month-row';
    ...
    const bufferSpent = getBufferExpenseSumForMonth(currentMonth);
    const bufferOpen = getBufferExpenseOpenSumForMonth(currentMonth);
    const totalAvail = totalAvailBeforeBuffer - bufferSpent;
    const remainingBuffer = getRemainingMinBufferForMonth(currentMonth);
    const totalExpenses = totalCommonRounded + totalPersonal;

    function euro(value) {
      return `${Number(value || 0).toFixed(2)} €`;
    }

    function createOverviewMetric(label, value, hint, kind, big) {
      const box = document.createElement('div');
      box.className = `overview-metric${big ? ' big' : ''}${kind ? ` ${kind}` : ''}`;
      ...
      if (hint) {
        ...
        hintEl.className = 'overview-metric-hint';
    ...
    mainGrid.appendChild(createOverviewMetric(
      'Aktuell verfügbar',
      euro(totalAvail),
      `Vor Puffer: ${euro(totalAvailBeforeBuffer)} · Rest-Puffer: ${euro(remainingBuffer)}`,
      totalAvail >= savingsConfig.minFree ? 'success' : 'warning',
      true
    ));
    mainGrid.appendChild(createOverviewMetric('Netto gesamt', euro(totalIncome), `Monat ${formatMonthLabel(currentMonth)}`, '', false));
    mainGrid.appendChild(createOverviewMetric('Gemeinsame Kosten', euro(totalCommonRounded), 'gerundete Anteile', '', false));
    mainGrid.appendChild(createOverviewMetric('Persönliche Ausgaben', euro(totalPersonal), `Kosten gesamt: ${euro(totalExpenses)}`, '', false));
    card.appendChild(mainGrid);

    const bufferBand = document.createElement('div');
    bufferBand.className = 'overview-buffer-band';
    const bufferTitle = document.createElement('div');
    bufferTitle.className = 'overview-buffer-title';
    const titleStrong = document.createElement('strong');
    titleStrong.textContent = 'Mindestpuffer';
    const titleText = document.createElement('span');
    titleText.textContent = 'nur bezahlte Puffer-Ausgaben reduzieren den Rest-Puffer';
    bufferTitle.appendChild(titleStrong);
    bufferTitle.appendChild(titleText);
    bufferBand.appendChild(bufferTitle);
    const bufferStats = document.createElement('div');
    bufferStats.className = 'overview-buffer-stats';
    bufferStats.appendChild(createOverviewMetric('Bezahlt', euro(bufferSpent), '', bufferSpent > 0 ? 'warning' : '', false));
    bufferStats.appendChild(createOverviewMetric('Offen geplant', euro(bufferOpen), '', bufferOpen > 0 ? 'warning' : '', false));
    bufferStats.appendChild(createOverviewMetric('Rest', euro(remainingBuffer), '', remainingBuffer > 0 ? 'success' : 'warning', false));
    const bufferNavButton = document.createElement('button');
    bufferNavButton.textContent = 'Mindestpuffer öffnen';
    bufferNavButton.className = 'secondary compact';
    bufferNavButton.addEventListener('click', () => {
      currentSection = 'buffer';
      render();
    });
    const bufferAction = document.createElement('div');
    bufferAction.className = 'overview-buffer-action';
    bufferAction.appendChild(bufferNavButton);
    bufferStats.appendChild(bufferAction);
    bufferBand.appendChild(bufferStats);
    card.appendChild(bufferBand);

    const note = document.createElement('p');
    note.className = 'small muted overview-note';
    note.textContent = 'Netto-Anpassungen öffnest du über „Bearbeiten“. Der Monat springt beim Monatswechsel automatisch auf den aktuellen Monat.';
    card.appendChild(note);

    const personWrap = document.createElement('div');
    personWrap.className = 'overview-person-grid';
    personsData.forEach((pd) => {
      const available = pd.income - pd.commonShare - pd.personalDue;
      const personCard = document.createElement('div');
      personCard.className = 'overview-person-card';
      const top = document.createElement('div');
      top.className = 'overview-person-top';
      const name = document.createElement('h3');
      name.textContent = pd.person.name;
      const editBtn = document.createElement('button');
      editBtn.textContent = 'Bearbeiten';
      editBtn.className = 'primary compact';
      editBtn.addEventListener('click', () => showPersonIncomeEditor(pd.person));
      top.appendChild(name);
      top.appendChild(editBtn);
      personCard.appendChild(top);

      const personStats = document.createElement('div');
      personStats.className = 'overview-person-stats';
      [
        ['Netto', euro(pd.income)],
        ['Gemeinsame Kosten', euro(pd.commonShare)],
        ['Persönliche Ausgaben', euro(pd.personalDue)],
        ['Verfügbar vor Puffer', euro(available)]
      ].forEach(([label, value]) => {
        const item = document.createElement('div');
        item.className = 'overview-person-stat';
        ...
        personStats.appendChild(item);
      });
      personCard.appendChild(personStats);
      personWrap.appendChild(personCard);
    });
    card.appendChild(personWrap);

    overviewSection.appendChild(card);
    overviewSection.appendChild(renderDataCheckCard());
  }
  // Rendert den Bereich „Mindestpuffer“
  function renderBufferExpenses() {
    ...
    const remaining = getRemainingMinBufferForMonth(currentMonth);
    ...
    card.appendChild(createSummaryMetrics([
      ...
      { label: 'Rest-Puffer', value: `${remaining.toFixed(2)} €`, kind: remaining > 0 ? 'success' : 'warning' }
    ]));

    const hint = document.createElement('p');
    hint.className = 'small muted';
    hint.textContent = 'Hier trägst du Ausgaben ein, die vom 150€-Mindestpuffer bezahlt werden. Nur der Status „bezahlt“ reduziert den Rest-Puffer. Rücklagen & Sparen bleiben davon unberührt.';
    card.appendChild(hint);

    const posts = (state.bufferExpenses || []);
    if (posts.length === 0) {
      const empty = document.createElement('p');
      empty.textContent = 'Noch keine Puffer-Ausgaben eingetragen.';
...
function showPersonalEditor(personId, editPost) {
  ...
  function showBufferExpenseEditor(editPost) {
    ...
    const info = document.createElement('p');
    info.className = 'small muted';
    info.textContent = `Diese Ausgaben werden im Mindestpuffer vorgemerkt. Erst wenn du sie als bezahlt markierst, reduzieren sie den Rest-Puffer für ${formatMonthLabel(currentMonth)}. Rücklagen & Sparen werden dadurch nicht reduziert.`;
    content.appendChild(info);
    ...
    showModal(editPost ? 'Puffer-Ausgabe bearbeiten' : 'Neue Puffer-Ausgabe', content, [
      {
        label: 'Abbrechen',
        className: 'secondary',
        onClick: (close) => close()
  ...
  function renderTankCalc() {
    ...
    keyHint.textContent = 'Der API-Key wird lokal gespeichert und in der Ansicht maskiert. Im Backup kannst du entscheiden, ob er enthalten sein soll.';
  ...
  function getDebtPaymentAmountForMonth(debt, monthKey) {
    ...
    return debt.paidMonths.includes(monthKey) ? Number(debt.monthlyRate || 0) : 0;
  }
  ...
  function addDebtPayment(debt, options = {}) {
    ...
    const amount = Number.isFinite(amountRaw) ? Math.max(0, amountRaw) : Number(debt.monthlyRate || 0);
  ...
  function resetDebtPaymentForMonth(debt, monthKey) {
    ...
    if (entries.length > 0) {
      ...
      restoreAmount = Number(debt.monthlyRate || 0);
    }
  ...
  function renderDebts() {
    ...
    state.debts.forEach((d) => {
      ...
      if (Number(d.amountOpen || 0) > 0) activeDebts += 1;
      if (d.nextDueMonth === currentMonth && Number(d.monthlyRate || 0) > 0) {
    ...
    const zeroRateDebts = state.debts.filter((d) => Number(d.amountOpen || 0) > 0 && !(Number(d.monthlyRate || 0) > 0)).length;
    card.appendChild(createSummaryMetrics([
      { label: 'Restschuld gesamt', value: `${totalDebtSum.toFixed(2)} €`, kind: totalDebtSum > 0 ? 'danger' : 'success' },
      { label: 'Rate diesen Monat', value: `${dueSum.toFixed(2)} €`, kind: dueSum > 0 ? 'warning' : '' },
      ...
      { label: 'Ohne Rate', value: String(zeroRateDebts), kind: zeroRateDebts > 0 ? 'warning' : 'success' }
    ]));
    ...
    if (state.debts.length === 0) {
      ...
      table.className = 'list-table';
      const thead = document.createElement('thead');
      thead.innerHTML = `<tr><th>Name</th><th>Offen</th><th>Monatsrate</th><th>Nächste Fälligkeit</th><th>Vorauss. Ende</th><th>Status</th><th>Bezahlt?</th><th>Aktion</th></tr>`;
      table.appendChild(thead);
      ...
      state.debts.forEach((d) => {
        ...
        if (!(Number(d.monthlyRate || 0) > 0) && Number(d.amountOpen || 0) > 0) statusHtml = '<span class="pill warning">Ratenplan offen</span>';
        ...
        tr.innerHTML = `<td>${d.name}</td><td>${Number(d.amountOpen || 0).toFixed(2)} €</td><td>${Number(d.monthlyRate || 0).toFixed(2)} €</td><td>${d.nextDueMonth || '-'}</td><td>${estimatedEnd || '-'}</td><td>${statusHtml}</td><td></td><td></td>`;

        const payCell = tr.children[6];
        if (paidNow) {
        ...
        } else if (dueNow && Number(d.monthlyRate || 0) > 0) {
          const btn = document.createElement('button');
          btn.textContent = 'Markieren';
          btn.className = 'success';
          btn.addEventListener('click', () => {
      ...
      const info = document.createElement('p');
      info.className = 'small muted';
      info.innerHTML = `<strong>In diesem Monat bereits bezahlt:</strong> ${paidSum.toFixed(2)} € · <strong>Noch zu bezahlen:</strong> ${openThisMonth.toFixed(2)} €`;
  ...
  function markDebtPaid(debt) {
    ensureDebtConfig(debt);
    if (!(Number(debt.monthlyRate || 0) > 0)) {
    ...
    if (addDebtPayment(debt, {
      month: paidMonth,
      amount: Number(debt.monthlyRate || 0),
  ...
  function showDebtPaymentEditor(debt) {
    ...
    const info = document.createElement('p');
    info.className = 'small muted';
    info.innerHTML = `<strong>${debt.name}</strong> · Offen: ${Number(debt.amountOpen || 0).toFixed(2)} € · Rate: ${Number(debt.monthlyRate || 0).toFixed(2)} €`;
    content.appendChild(info);
    ...
    refs.amountInput.type = 'number';
    refs.amountInput.step = '0.01';
    refs.amountInput.value = Number(debt.monthlyRate || 0) > 0 ? Number(debt.monthlyRate || 0).toFixed(2) : '';
    ...
    showModal('Zahlung eintragen', content, [
      { label: 'Abbrechen', className: 'secondary', onClick: (close) => close() },
      {
        ...
        onClick: (close) => {
          ...
          if (markAsMonthly && !(Number(debt.monthlyRate || 0) > 0)) return alert('Für eine Regelrate muss zuerst eine Monatsrate hinterlegt sein.');
          if (addDebtPayment(debt, {
            month,
            amount,
  ...
  function showDebtEditor(editDebt) {
    ...
    const hint = document.createElement('p');
    hint.className = 'small muted';
    hint.textContent = 'Monatsrate 0 € ist erlaubt, wenn noch kein Ratenplan feststeht. Dann zeigt die App „Ratenplan offen“ und markiert keine Regelrate als fällig.';
    content.appendChild(hint);
    ...
    showModal(editDebt ? 'Schuld bearbeiten' : 'Neue Schuld anlegen', content, [
      ...
      },
      {
        ...
        onClick: (close) => {
          ...
          const rate = parseFloat(refs.rateInput.value);
          const due = refs.dueInput.value;
          if (!name) return alert('Name darf nicht leer sein.');
          if (Number.isNaN(open) || open < 0) return alert('Bitte einen gültigen offenen Betrag eingeben.');
          if (Number.isNaN(rate) || rate < 0) return alert('Bitte eine gültige Monatsrate eingeben. 0 € ist erlaubt, wenn noch kein Ratenplan feststeht.');
          ...
          if (editDebt) {
            ...
            state.debts.push({ id: generateId(), name, amountOpen: open, monthlyRate: rate, nextDueMonth: due, paidMonths: [] });
          }
          saveState();
          render();
          close();
...
function renderSavings() {
    ...
    card.appendChild(createSummaryMetrics([
      { label: 'Freier Betrag', value: `${free.toFixed(2)} €`, kind: free >= savingsConfig.minFree ? 'success' : 'warning' },
      { label: 'Puffer bezahlt', value: `${bufferSpent.toFixed(2)} €`, kind: bufferSpent > 0 ? 'warning' : '' },
      { label: 'Rest-Puffer', value: `${remainingBuffer.toFixed(2)} €`, kind: remainingBuffer > 0 ? 'success' : 'warning' },
      ...
      { label: 'Verteilbar', value: `${verteilbar.toFixed(2)} €`, kind: verteilbar > 0 ? 'success' : 'warning' }
    ]));
    ...
    if (beforeStart) {
      ...
      info.innerHTML = `Der Mindestpuffer von <strong>${savingsConfig.minFree.toFixed(2)} €</strong> bleibt geschützt. Nur bezahlte Puffer-Ausgaben werden gegen diesen Puffer gerechnet.`;
    }
    card.appendChild(info);
...
function renderPots() {
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.79 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.79 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.79 geladen';
    }
