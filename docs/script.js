/*
 * Haushaltsplaner Developer Beta 0.96
 *
...
(() => {
  ...
  function getSnowballExcludeReason(debt) {
    ...
    if (!(Number(debt.monthlyRate || 0) > 0)) return 'kein Ratenplan hinterlegt';
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV096', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    try {
      ...
      localStorage.setItem('budgetStateV096', payload);
  ...
  function getDebtMonthAmount(debt, monthKey) {
    ...
    return monthDiff(debt.nextDueMonth, monthKey) >= 0 ? rate : 0;
  }
  ...
  function buildSnowballPlan(startMonth = currentMonth, maxMonths = 72) {
    const sourceDebts = (state.debts || []).map((debt) => {
      ...
      return {
        ...
        rate: Math.max(0, Number(debt.monthlyRate || 0)),
    ...
    for (let i = 0; i < maxMonths && active.some((debt) => debt.open > 0); i += 1) {
      ...
      rows.push({ month, base, extra, total: base + extra, rolloverNext: rollover + newlyFreed, remaining, notes: notes.slice(0, 4).join(' · ') });
      if (remaining <= 0.005) {
        debtFreeMonth = month;
        break;
      }
...
function showPersonalEditor(personId, editPost) {
  ...
  function renderTankCalc() {
    ...
    apiStatusBox.innerHTML = `<strong>Tankerkönig-Status:</strong> ${state.tankCalc.apiKey ? 'API-Key vorhanden' : 'API-Key fehlt'} · Letzter Abruf: ${apiStatus.lastRequestLabel}<br><span class="muted">${apiStatus.lastStatus ? apiStatus.lastStatus + ': ' : ''}${apiStatus.lastError || 'Noch keine Diagnose durchgeführt.'}</span>`;
  ...
  function renderDebts() {
    ...
    const estimatedDebtFree = state.debts
      .filter((d) => Number(d.amountOpen || 0) > 0 && d.paymentType === 'installment' && Number(d.monthlyRate || 0) > 0)
    ...
    if (visibleDebts.length === 0) {
      ...
      table.className = 'list-table';
      const thead = document.createElement('thead');
      thead.innerHTML = `<tr><th>Name</th><th>Zahlungsart</th><th>Offen</th><th>Monatsrate</th><th>Nächste Fälligkeit</th><th>Vorauss. Ende</th><th>Fortschritt</th><th>Status</th><th>Bezahlt?</th><th>Aktion</th></tr>`;
      table.appendChild(thead);
      ...
      visibleDebts.forEach((d) => {
        ...
        tr.innerHTML = `<td>${d.name}</td><td>${typeHtml}</td><td>${Number(d.amountOpen || 0).toFixed(2)} €</td><td>${Number(d.monthlyRate || 0).toFixed(2)} €</td><td>${d.nextDueMonth || '-'}</td><td>${estimatedEnd || '-'}</td><td>${progressHtml}</td><td>${statusHtml}</td><td></td><td></td>`;

        const payCell = tr.children[8];
  ...
  function renderSnowballPlanCard(monthKey) {
    ...
    const p = document.createElement('p');
    p.className = 'small muted';
    p.textContent = 'Berücksichtigt werden nur echte laufende Ratenzahlungen. Einmalzahlungen wie Riverty AZ2 oder einmalige Restzahlungen werden nicht als Schneeball-Ziel genutzt. Wenn eine Schuld z. B. im Juni abbezahlt ist, wird ihre normale Rate erst ab Juli auf die dann kleinste offene Ratenschuld umgelegt.';
    card.appendChild(p);
    ...
    const table = document.createElement('table');
    table.className = 'list-table';
    table.innerHTML = '<thead><tr><th>Monat</th><th>Normale Raten</th><th>Schneeball extra</th><th>Gesamtzahlung</th><th>Zusatzrate ab Folgemonat</th><th>Rest nach Monat</th><th>Hinweis</th></tr></thead>';
    const tbody = document.createElement('tbody');
    plan.rows.slice(0, 18).forEach((row) => {
      const tr = document.createElement('tr');
      tr.innerHTML = `<td>${formatMonthLabel(row.month)}</td><td>${row.base.toFixed(2)} €</td><td>${row.extra.toFixed(2)} €</td><td>${row.total.toFixed(2)} €</td><td>${row.rolloverNext.toFixed(2)} €</td><td>${row.remaining.toFixed(2)} €</td><td>${row.notes || '-'}</td>`;
      tbody.appendChild(tr);
    });
    table.appendChild(tbody);
    card.appendChild(table);
  ...
  function showDebtEditor(editDebt) {
    ...
    const hint = document.createElement('p');
    hint.className = 'small muted';
    hint.textContent = 'Zahlungsart steuert den Schneeball: Nur Ratenzahlungen zählen mit. Einmalzahlungen und offene Ratenpläne werden nicht als Schneeball-Ziel genutzt.';
    content.appendChild(hint);
      ...
      },
      {
        ...
        onClick: (close) => {
          ...
          if (editDebt) {
            ...
            const newDebt = { id: generateId(), name, amountOpen: open, monthlyRate: rate, paymentType, nextDueMonth: due, paidMonths: [] };
...
function renderPots() {
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.96 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.96 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.96 geladen';
    }
