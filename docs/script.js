/*
 * Haushaltsplaner Developer Beta 0.82
 *
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV082', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    localStorage.setItem('budgetStateV082', JSON.stringify(state));
  }
...
function renderPots() {
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.82 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.82 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.82 geladen';
    }
