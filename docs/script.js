/*
 * Haushaltsplaner Developer Beta 0.76
 *
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV076', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    localStorage.setItem('budgetStateV076', JSON.stringify(state));
  }
...
function renderPots() {
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.76 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.76 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.76 geladen';
    }
