/*
 * Haushaltsplaner Developer Beta 0.85
 *
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV084', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    try {
      ...
      localStorage.setItem('budgetStateV084', payload);
...
2 = ab diesem Monat dauerhaft`,
  ...
  function renderOverview() {
    ...
    const bufferFulfilled = remainingBuffer <= 0;
    ...
    kpiGrid.appendChild(createKpi({
      ...
      hint: `vor Puffer: ${euro(totalAvailBeforeBuffer)}`,
      icon: '▣',
      hero: true,
    ...
    const bufferStrip = div('buffer-strip');
    bufferStrip.appendChild(createBufferItem('◈', 'Puffer bezahlt', euro(bufferSpent)));
    bufferStrip.appendChild(createBufferItem('◌', 'Puffer offen geplant', euro(bufferOpen)));
    bufferStrip.appendChild(createBufferItem('◎', 'Rest vom 150€-Puffer', euro(remainingBuffer)));
    const status = div(`buffer-status-chip${bufferFulfilled ? ' success' : ' warning'}`, bufferFulfilled ? 'Puffer erfüllt' : 'Puffer offen');
    status.addEventListener('click', () => {
      currentSection = 'buffer';
      render();
    });
    bufferStrip.appendChild(status);
    page.appendChild(bufferStrip);
    ...
    personsData.forEach((pd) => {
      ...
      availableBox.appendChild(div('person-modern-label', 'Verfügbar (vor Puffer)'));
    ...
    const bufferWidget = div('buffer-widget');
    const widgetTitle = div('buffer-widget-title');
    widgetTitle.appendChild(makeIcon('◈', 'tiny'));
    const widgetHeadline = document.createElement('h3');
    widgetHeadline.textContent = 'Mindestpuffer';
    widgetTitle.appendChild(widgetHeadline);
    bufferWidget.appendChild(widgetTitle);
    const progress = div('buffer-progress');
    const progressRing = div('buffer-progress-ring');
    const progressInner = div('buffer-progress-inner', '✓');
    progressRing.appendChild(progressInner);
    progress.appendChild(progressRing);
    const progressText = div('buffer-progress-text');
    progressText.appendChild(div('', `${savingsConfig.minFree.toFixed(2)} €`));
    progressText.appendChild(div('muted', 'Ziel'));
    progressText.appendChild(div('green-strong', `${Math.min(bufferSpent, savingsConfig.minFree + Math.max(0, bufferSpent - savingsConfig.minFree)).toFixed(2)} €`));
    progressText.appendChild(div('muted', 'erreicht'));
    progressText.appendChild(div('percent-big', bufferFulfilled ? '100 %' : `${Math.max(0, Math.min(100, (bufferSpent / savingsConfig.minFree) * 100)).toFixed(0)} %`));
    progress.appendChild(progressText);
    bufferWidget.appendChild(progress);
    const bufferMessage = div(`buffer-widget-message${bufferFulfilled ? ' success' : ' warning'}`, bufferFulfilled ? 'Dein Mindestpuffer ist vollständig abgedeckt.' : 'Der Mindestpuffer ist noch nicht vollständig abgedeckt.');
    bufferWidget.appendChild(bufferMessage);
    lower.appendChild(bufferWidget);
...
function renderPots() {
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.85 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.85 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.85 geladen';
    }
