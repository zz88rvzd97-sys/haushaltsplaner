/*
 * Haushaltsplaner Developer Beta 0.92
 *
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV092', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    try {
      ...
      localStorage.setItem('budgetStateV092', payload);
...
function renderPots() {
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.92 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.92 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.92 geladen';
    }
