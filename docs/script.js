/*
 * Haushaltsplaner Developer Beta 0.80
 *
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV080', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    localStorage.setItem('budgetStateV080', JSON.stringify(state));
  }
...
function showPersonalEditor(personId, editPost) {
  ...
  function renderDebts() {
    ...
    state.debts.forEach((d) => {
      ...
      const monthlyRate = Number(d.monthlyRate || 0);
...
function renderPots() {
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.80 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.80 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.80 geladen';
    }
