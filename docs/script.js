/*
 * Haushaltsplaner Developer Beta 0.95
 *
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV095', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    try {
      ...
      localStorage.setItem('budgetStateV095', payload);
...
function renderPots() {
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.95 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.95 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.95 geladen';
    }
