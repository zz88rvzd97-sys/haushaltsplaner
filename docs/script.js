/*
 * Haushaltsplaner Developer Beta 0.78
 *
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV078', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    localStorage.setItem('budgetStateV078', JSON.stringify(state));
  }
...
2 = ab diesem Monat dauerhaft`,
  ...
  function renderOverview() {
    ...
    info.textContent = 'Netto-Anpassungen öffnest du hier über „Bearbeiten“. Der Monat springt beim Monatswechsel automatisch auf den aktuellen Monat.';
    card.appendChild(info);

    card.appendChild(createSummaryMetrics([
      { label: 'Aktuell verfügbar', value: `${totalAvail.toFixed(2)} €`, kind: totalAvail >= savingsConfig.minFree ? 'success' : 'warning', hint: 'nach bezahlten Puffer-Ausgaben' },
      { label: 'Verfügbar vor Puffer', value: `${totalAvailBeforeBuffer.toFixed(2)} €`, kind: totalAvailBeforeBuffer >= savingsConfig.minFree ? 'success' : 'warning', hint: 'Basis für Rücklagen & Sparen' },
      { label: 'Netto gesamt', value: `${totalIncome.toFixed(2)} €` },
      { label: 'Gemeinsame Kosten', value: `${totalCommonRounded.toFixed(2)} €` },
      { label: 'Persönliche Ausgaben', value: `${totalPersonal.toFixed(2)} €` },
      { label: 'Puffer bezahlt', value: `${bufferSpent.toFixed(2)} €`, kind: bufferSpent > 0 ? 'warning' : '' },
      { label: 'Puffer offen geplant', value: `${bufferOpen.toFixed(2)} €`, kind: bufferOpen > 0 ? 'warning' : '' },
      { label: 'Rest vom 150€-Puffer', value: `${remainingBuffer.toFixed(2)} €`, kind: remainingBuffer < savingsConfig.minFree ? 'warning' : 'success' }
    ]));
    ...
    footRow.innerHTML = `<td><strong>Summe</strong></td>
      ...
      <td>${totalAvailBeforeBuffer.toFixed(2)} €</td>
      <td>-</td>`;
    ...
    const bufferShortcut = document.createElement('div');
    bufferShortcut.className = 'card';
    bufferShortcut.innerHTML = `<h2>Mindestpuffer</h2>
      <p class="small muted">Geplante Puffer-Ausgaben werden jetzt separat verwaltet. Nur als bezahlt markierte Ausgaben reduzieren den Rest-Puffer.</p>`;
    bufferShortcut.appendChild(createSummaryMetrics([
      { label: 'Bezahlt aus Puffer', value: `${bufferSpent.toFixed(2)} €`, kind: bufferSpent > 0 ? 'warning' : 'success' },
      { label: 'Noch offen geplant', value: `${bufferOpen.toFixed(2)} €`, kind: bufferOpen > 0 ? 'warning' : '' },
    ...
    bufferNavButton.className = 'primary';
    bufferNavButton.addEventListener('click', () => {
      currentSection = 'buffer';
      render();
    });
    bufferShortcut.appendChild(bufferNavButton);
    overviewSection.appendChild(bufferShortcut);
  }
...
function renderPots() {
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.78 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.78 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.78 geladen';
    }
