/*
 * Haushaltsplaner Developer Beta 0.74
 *
...
(() => {
  ...
  function syncDebtPaymentFromPost(post, monthKey) {
    ...
    if (!debt.paidMonths.includes(monthKey)) debt.paidMonths.push(monthKey);
    debt.amountOpen = Math.max(0, Number(debt.amountOpen || 0) - Number(debt.monthlyRate || 0));
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV074', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    localStorage.setItem('budgetStateV074', JSON.stringify(state));
  }

  function normalizeAppMeta() {
    if (!state.appMeta || typeof state.appMeta !== 'object') state.appMeta = { selectedMonth: '', lastAutoMonthCheck: '' };
...
function showPersonalEditor(personId, editPost) {
  ...
  function renderTankCalc() {
    ...
    keyHint.textContent = 'Der API-Key wird lokal im Browser gespeichert und bleibt nach dem Sichern erhalten.';
  ...
  function getDebtPaidAmountForMonth(monthKey) {
    ...
    state.debts.forEach((d) => {
      ensureDebtConfig(d);
      if (d.paidMonths.includes(monthKey)) paid += Number(d.monthlyRate || 0);
    });
    return paid;
  }
    ...
    if (state.debts.length === 0) {
      ...
      state.debts.forEach((d) => {
        ...
        if (paidNow) {
          ...
          done.className = 'pill success';
          done.textContent = 'Bezahlt';
  ...
  function markDebtPaid(debt) {
    ...
    if (isMonthKey(paidMonth) && !debt.paidMonths.includes(paidMonth)) debt.paidMonths.push(paidMonth);
  ...
  function showDebtEditor(editDebt) {
    ...
    const hint = document.createElement('p');
    hint.className = 'small muted';
    hint.textContent = 'So bleibt der Schuldenbereich kompakt – Details öffnest du nur bei Bedarf.';
    content.appendChild(hint);
    ...
    showModal(editDebt ? 'Schuld bearbeiten' : 'Neue Schuld anlegen', content, [
      ...
      },
      {
        ...
        onClick: (close) => {
          ...
          if (Number.isNaN(rate) || rate <= 0) return alert('Bitte eine gültige Monatsrate eingeben.');
...
function renderPots() {
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.74 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.74 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.74 geladen';
    }
