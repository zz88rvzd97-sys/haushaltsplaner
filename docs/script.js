/*
 * Haushaltsplaner Developer Beta 0.75
 *
...
(() => {
  ...
  let state;
  try {
  let saved = localStorage.getItem('budgetStateV075');
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV075', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    localStorage.setItem('budgetStateV075', JSON.stringify(state));
  }
...
2 = ab diesem Monat dauerhaft`,
  ...
  function renderOverview() {
    ...
    card.appendChild(createSummaryMetrics([
      ...
      { label: 'Puffer-Ausgaben', value: `${bufferSpent.toFixed(2)} €`, kind: bufferSpent > 0 ? 'warning' : '' },
      { label: 'Frei nach Puffer', value: `${totalAvail.toFixed(2)} €`, kind: totalAvail < savingsConfig.minFree ? 'danger' : 'success' },
    ...
    bufferCard.className = 'card';
    const bufferHeader = document.createElement('div');
    bufferHeader.className = 'row';
    const bufferTitle = document.createElement('h2');
    bufferTitle.textContent = 'Ausgaben aus dem Mindestpuffer';
    bufferTitle.style.flex = '1 1 auto';
    const bufferBtn = document.createElement('button');
    bufferBtn.textContent = '+ Puffer-Ausgabe';
    bufferBtn.className = 'primary';
    bufferBtn.addEventListener('click', () => showBufferExpenseEditor());
    bufferHeader.appendChild(bufferTitle);
    bufferHeader.appendChild(bufferBtn);
    bufferCard.appendChild(bufferHeader);

    const bufferHint = document.createElement('p');
    bufferHint.className = 'small muted';
    bufferHint.textContent = 'Diese Posten werden direkt vom 150€-Mindestpuffer abgezogen. Rücklagen & Sparen bleiben davon unberührt; angezeigt wird nur, wie viel vom Puffer noch übrig ist.';
    bufferCard.appendChild(bufferHint);

    const bufferPosts = (state.bufferExpenses || []);
    if (bufferPosts.length === 0) {
      const empty = document.createElement('p');
      empty.textContent = 'Noch keine Puffer-Ausgaben eingetragen.';
      bufferCard.appendChild(empty);
    } else {
      const bufferTable = document.createElement('table');
      bufferTable.className = 'list-table';
      bufferTable.innerHTML = `<thead><tr><th>Name</th><th>Betrag</th><th>Intervall</th><th>Start</th><th>Bis</th><th>Fällig</th><th>Aktion</th></tr></thead>`;
      const tbody = document.createElement('tbody');
      bufferPosts.forEach((post) => {
        const tr = document.createElement('tr');
        const dueNow = isDue(post, currentMonth);
        tr.innerHTML = `<td>${post.name}</td>
          <td>${getEffectiveAmountForMonth(post, currentMonth).toFixed(2)} €</td>
      ...
      bufferTable.appendChild(tbody);
      bufferCard.appendChild(bufferTable);
    }
    overviewSection.appendChild(bufferCard);
  }
...
function showPersonalEditor(personId, editPost) {
  ...
  function showBufferExpenseEditor(editPost) {
    ...
    const info = document.createElement('p');
    info.className = 'small muted';
    info.textContent = `Diese Ausgaben gehen direkt vom Mindestpuffer im Monat ${formatMonthLabel(currentMonth)} ab. Rücklagen & Sparen werden dadurch nicht reduziert.`;
    content.appendChild(info);
...
function renderSavings() {
    ...
    if (beforeStart) {
      ...
      info.innerHTML = `Der Mindestpuffer von <strong>${savingsConfig.minFree.toFixed(2)} €</strong> bleibt geschützt. Puffer-Ausgaben werden nur gegen diesen Puffer gerechnet.`;
    }
    card.appendChild(info);
...
function renderPots() {
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.75 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.75 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.75 geladen';
    }
