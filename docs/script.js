/*
 * Haushaltsplaner Developer Beta 0.91
 *
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV091', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    try {
      ...
      localStorage.setItem('budgetStateV091', payload);
...
2 = ab diesem Monat dauerhaft`,
  ...
  function renderForecast() {
    ...
    months.forEach(({ key, label }) => {
      const details = computeMonthDetails(key);
      let incomeAdjustment = 0;
      state.persons.forEach((person) => {
        const raw = scenarioNet[person.id];
        if (raw !== '' && raw != null && Number.isFinite(Number(raw))) {
          incomeAdjustment += Number(raw) - getPersonNet(person, key);
        }
      });
      const totalIncome = details.totalIncome + incomeAdjustment;
      const free = details.free + incomeAdjustment;
      const distributable = Math.max(free, 0);
      const tr = document.createElement('tr');
      tr.innerHTML = `<td>${label}</td><td>${totalIncome.toFixed(2)} €</td><td>${details.totalCommonRounded.toFixed(2)} €</td><td>${details.totalPersonal.toFixed(2)} €</td><td>${details.miscPaid.toFixed(2)} €</td><td><span class="pill ${free < 0 ? 'danger' : 'success'}">${free.toFixed(2)} €</span></td><td>${(distributable * savingsConfig.reservesRatio).toFixed(2)} €</td><td>${(distributable * savingsConfig.savingsRatio).toFixed(2)} €</td>`;
      tbody.appendChild(tr);
    });
    table.appendChild(tbody);
    card.appendChild(table);
...
function renderPots() {
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.91 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.91 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.91 geladen';
    }
