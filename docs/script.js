/*
 * Haushaltsplaner Developer Beta 0.88
 *
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV088', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    try {
      ...
      localStorage.setItem('budgetStateV088', payload);
...
2 = ab diesem Monat dauerhaft`,
  ...
  function renderWarningsCard(monthKey) {
    ...
    h.textContent = 'Hinweise für diesen Monat';
    card.appendChild(h);
  ...
  function renderChangeLogCard(limit = 10) {
    ...
    state.changeLog.slice(0, limit).forEach((entry) => {
...
function renderPots() {
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.88 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.88 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.88 geladen';
    }
