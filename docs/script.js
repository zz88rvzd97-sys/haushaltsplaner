/*
 * Haushaltsplaner Developer Beta 0.90
 *
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV090', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    try {
      ...
      localStorage.setItem('budgetStateV090', payload);
...
function renderPots() {
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.90 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.90 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.90 geladen';
    }
