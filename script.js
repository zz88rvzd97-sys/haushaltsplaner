/*
 * Haushaltsplaner Developer Beta 0.45
 *
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV045', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    localStorage.setItem('budgetStateV045', JSON.stringify(state));
  }
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.45 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.45 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.45 geladen';
    }
