/*
 * Haushaltsplaner Developer Beta 0.66
 *
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV066', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    localStorage.setItem('budgetStateV066', JSON.stringify(state));
  }
...
function renderPots() {
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.66 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.66 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.66 geladen';
    }
