/*
 * Haushaltsplaner Developer Beta 0.72
 *
 * Diese Version bringt das Kennzahlen-Layout aus der Übersicht
 * in die weiteren Hauptbereiche. Schulden, Regeln & Personen,
 * Rücklagen & Sparen, Töpfe, Tankgeld und Sichern zeigen damit
 * wichtige Summen sofort oben als kompakte Kacheln.
 */
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV072', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    localStorage.setItem('budgetStateV072', JSON.stringify(state));
  }
...
function showCommonEditor(editCost) {
    ...
    hint.textContent = editCost
      ...
      : 'Neue Posten werden direkt mit ihren Laufzeitregeln gespeichert.';
...
function showPersonalEditor(personId, editPost) {
    ...
    hint.textContent = editPost
      ...
      : 'Neue persönliche Posten kannst du hier kompakt anlegen.';
    content.appendChild(hint);
      ...
      },
      {
        ...
        onClick: (close) => {
          ...
          if (editDebt) {
            ...
            state.debts.push({ id: generateId(), name, amountOpen: open, monthlyRate: rate, nextDueMonth: due });
          }
          saveState();
          render();
          close();
...
function renderPots() {
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.72 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.72 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.72 geladen';
    }
