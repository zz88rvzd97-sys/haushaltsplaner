/*
 * Haushaltsplaner Developer Beta 0.57
 *
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV057', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    localStorage.setItem('budgetStateV057', JSON.stringify(state));
  }
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.57 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.57 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.57 geladen';
    }
