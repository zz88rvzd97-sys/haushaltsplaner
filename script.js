/*
 * Haushaltsplaner Developer Beta 0.63
 *
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV063', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    localStorage.setItem('budgetStateV063', JSON.stringify(state));
  }
    ...
    if (state.commonCosts.length === 0) {
      ...
      state.commonCosts.forEach((c) => {
        ...
        editBtn.addEventListener('click', () => {
          ...
          saveBtn.addEventListener('click', () => {
            ...
            if (!newInterval || newInterval < 1) return alert('Intervall muss mindestens 1 betragen.');
            if (!/^\d{4}-\d{2}$/.test(newStart)) return alert('Startmonat muss im Format JJJJ-MM vorliegen.');
            if (newEnd && !/^\d{4}-\d{2}$/.test(newEnd)) return alert('Endmonat muss im Format JJJJ-MM vorliegen.');
            if (newEnd && monthDiff(newStart, newEnd) < 0) return alert('Endmonat darf nicht vor dem Startmonat liegen.');
            c.name = newName;
            c.amount = newAmount;
      ...
      state.commonCosts.forEach((c) => {
        if (isDue(c, currentMonth)) {
          dueSum += c.amount;
          if (c.paidMonths && c.paidMonths.includes(currentMonth)) paidSum += c.amount;
        }
      });
      if (dueSum > 0) {
...
function showCommonEditor(editCost) {
    ...
    let amount;
    if (editCost && (amountStr === '' || amountStr === null)) amount = editCost.amount;
    else {
    ...
    let interval;
    if (editCost && (intervalStr === '' || intervalStr === null)) interval = editCost.interval;
    else {
...
function showPersonalEditor(personId, editPost) {
    ...
    let amount;
    if (editPost && (amountStr === '' || amountStr === null)) amount = editPost.amount;
    else {
      amount = parseFloat(amountStr);
      if (isNaN(amount)) return;
    }
    ...
    let interval;
    if (editPost && (intervalStr === '' || intervalStr === null)) interval = editPost.interval;
    else {
      interval = parseInt(intervalStr, 10);
      if (!interval || interval < 1) return;
    }
...
function renderPots() {
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.63 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.63 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.63 geladen';
    }
