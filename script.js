/*
 * Haushaltsplaner Developer Beta 0.47
 *
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV047', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    localStorage.setItem('budgetStateV047', JSON.stringify(state));
  }
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.47 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.47 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.47 geladen';
    }
