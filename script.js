/*
 * Haushaltsplaner Developer Beta 0.67
 *
 * Diese Version behebt den Render-Fehler in der Übersicht, korrigiert die Versionsanzeige und ergänzt für gemeinsame und persönliche Posten eine
 * klare Auswahl zwischen einmaliger Zahlung und laufendem Posten sowie
 * eine optionale Befristung bis zu einem Endmonat. Außerdem wird der
 * Status „Fällig“ jetzt als auffällige rote bzw. grüne Markierung
 * dargestellt.
 *
 * Die monatsspezifischen Betragsänderungen aus Beta 0.64 bleiben
 * erhalten: Änderungen können weiterhin nur für den gewählten Monat
 * oder ab diesem Monat dauerhaft gespeichert werden.
 */
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV067', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    localStorage.setItem('budgetStateV067', JSON.stringify(state));
  }
...
function renderPots() {
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.67 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.67 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.67 geladen';
    }
