/*
 * Haushaltsplaner Developer Beta 0.43
 *
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV043', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    localStorage.setItem('budgetStateV043', JSON.stringify(state));
  }
  ...
  function renderDebts() {
    ...
    if (state.debts.length === 0) {
      ...
      if (dueSum > 0 || paidSum > 0) {
        ...
        if (open < 0) open = 0;
        info.innerHTML = `<strong>In diesem Monat bereits bezahlt:</strong> ${paidSum.toFixed(2)} € · <strong>Noch zu bezahlen:</strong> ${open.toFixed(2)} €`;
        card.appendChild(info);
      }
    }
