/*
 * Haushaltsplaner Developer Beta 0.38
 *
 * Diese Version zeigt im Bereich „Rücklagen & Sparen“ nur noch den
 * ausgewählten Monat statt aller 12 Monate gleichzeitig. Die einzelnen
 * Rücklagen-Töpfe sowie „Sparen“ stehen untereinander und können jeweils
 * separat als „zurückgelegt“ bzw. „gespart“ markiert werden.
 */
...
(() => {
  ...
  const defaultState = {
    ...
    debts: [],
    pots: [],
    // Sparen als separates Konto mit manuellem Saldo und Buchungen.
    savingsAccount: {
      balance: 0,
      transactions: []
    },
    // Alte Monats-Markierungen (wird bei Migration in reservesSavedEntries
    // überführt, damit einzelne Töpfe separat markiert werden können).
    reservesSavedMonths: [],
    // Einzelne Bestätigungen für Rücklagen-Topf bzw. Sparen, Format
    // `${month}|${item}`.
    reservesSavedEntries: []
  };
  let state;
  try {
  let saved = localStorage.getItem('budgetStateV038');
    if (!saved) {
      // Fallback-Migration aus älteren Versionen
      const fallback = [
        'budgetStateV037','budgetStateV036','budgetStateV035','budgetStateV034','budgetStateV033','budgetStateV032','budgetStateV031','budgetStateV030','budgetStateV029','budgetStateV028','budgetStateV027','budgetStateV026','budgetStateV025','budgetStateV024','budgetStateV023','budgetStateV022','budgetStateV021','budgetStateV020','budgetStateV019','budgetStateV018','budgetStateV017','budgetStateV016','budgetStateV015'
      ];
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV038', data);
          break;
        }
      }
    }
    state = saved ? JSON.parse(saved) : JSON.parse(JSON.stringify(defaultState));
    // Falls neue Felder in älteren Backups fehlen, initialisieren.
    if (!state.reservesSavedMonths) state.reservesSavedMonths = [];
    if (!state.reservesSavedEntries) state.reservesSavedEntries = [];
    if (!state.pots) state.pots = [];
    if (!state.savingsAccount) state.savingsAccount = { balance: 0, transactions: [] };
    if (!state.savingsAccount.transactions) state.savingsAccount.transactions = [];
    // Alte Monats-Markierungen in neue Einzelmarkierungen migrieren.
    if (state.reservesSavedMonths.length && !state.reservesSavedEntries.length) {
      const items = ['Auto', 'Urlaub', 'Anschaffungen (inkl. Wohnen)', 'Kleidung', 'Freizeit', 'Sparen'];
      state.reservesSavedMonths.forEach((month) => {
        items.forEach((item) => {
          state.reservesSavedEntries.push(`${month}|${item}`);
        });
      });
    }
  } catch (err) {
  ...
  function saveState() {
    localStorage.setItem('budgetStateV038', JSON.stringify(state));
  }
  ...
  function isSavingsActiveMonth(monthKey) {
    return monthDiff(savingsConfig.startMonth, monthKey) >= 0;
  }

  function getSavingsEntryKey(monthKey, itemName) {
    return `${monthKey}|${itemName}`;
  }

  function isSavingsItemSaved(monthKey, itemName) {
    if (!state.reservesSavedEntries) state.reservesSavedEntries = [];
    return state.reservesSavedEntries.includes(getSavingsEntryKey(monthKey, itemName));
  }

  function toggleSavingsItemSaved(monthKey, itemName) {
    if (!state.reservesSavedEntries) state.reservesSavedEntries = [];
    const key = getSavingsEntryKey(monthKey, itemName);
    const idx = state.reservesSavedEntries.indexOf(key);
    if (idx >= 0) {
      state.reservesSavedEntries.splice(idx, 1);
    } else {
      state.reservesSavedEntries.push(key);
    }
  }

  function ensureDefaultReservePots() {
    if (!state.pots) state.pots = [];
    const existingNames = new Set(state.pots.map((p) => p.name));
    let changed = false;
    Object.keys(savingsConfig.reservePotShares).forEach((name) => {
      if (!existingNames.has(name)) {
        state.pots.push({ id: generateId(), name, balance: 0, transactions: [] });
        changed = true;
      }
    });
    return changed;
  ...
  // Standard-Rücklagen-Töpfe immer vorhanden halten.
  if (ensureDefaultReservePots()) {
    saveState();
  }
  ...
  function getReserveContributionForPot(potName, monthKey) {
    if (!isSavingsActiveMonth(monthKey)) {
      return 0;
    }
  ...
  function estimateDebtPaidOffMonth(debt) {
    ...
    if (rate <= 0) return 'Offen / keine Rate';
    const monthsNeeded = Math.ceil(open / rate);
    const payoffKey = addMonthsToKey(debt.nextDueMonth, Math.max(monthsNeeded - 1, 0));
    return monthKeyToDate(payoffKey).toLocaleDateString('de-DE', { month: 'long', year: 'numeric' });
  }
  ...
  function renderDebts() {
    ...
    if (state.debts.length === 0) {
      ...
      table.className = 'list-table';
      const thead = document.createElement('thead');
      thead.innerHTML = `<tr><th>Name</th><th>Offen</th><th>Monatsrate</th><th>Nächste Fälligkeit</th><th>Vorauss. erledigt</th><th>Bezahlt?</th><th>Aktion</th></tr>`;
      table.appendChild(thead);
      const tbody = document.createElement('tbody');
      state.debts.forEach((d) => {
        const tr = document.createElement('tr');
        const dueNow = d.nextDueMonth === currentMonth;
        const payoffLabel = estimateDebtPaidOffMonth(d);
        tr.innerHTML = `<td>${d.name}</td><td>${d.amountOpen.toFixed(2)} €</td><td>${d.monthlyRate.toFixed(2)} €</td><td>${d.nextDueMonth}</td><td>${payoffLabel}</td><td></td><td></td>`;
  ...
  function renderSavings() {
    ...
    const card = document.createElement('div');
    card.className = 'card';
    // Monatsauswahl: zeigt nur den ausgewählten Monat
    const monthRow = document.createElement('div');
    monthRow.className = 'row';
    ...
    const verteilbar = Math.max(free - savingsConfig.minFree, 0);
    const active = isSavingsActiveMonth(currentMonth);
    const ruecklagen = active ? verteilbar * savingsConfig.reservesRatio : 0;
    const sparen = active ? verteilbar * savingsConfig.savingsRatio : 0;
    const monthLabel = monthKeyToDate(currentMonth).toLocaleDateString('de-DE', { month: 'long', year: 'numeric' });
    info.innerHTML = `<strong>${monthLabel}</strong><br>Freier Betrag: ${free.toFixed(2)} € · Mindestpuffer: ${savingsConfig.minFree.toFixed(2)} € · Verteilbar: ${verteilbar.toFixed(2)} €`;
    card.appendChild(info);

    if (!active) {
      const startInfo = document.createElement('p');
      startInfo.textContent = `Die Rücklagen- und Sparverteilung startet erst ab ${monthKeyToDate(savingsConfig.startMonth).toLocaleDateString('de-DE', { month: 'long', year: 'numeric' })}.`;
      card.appendChild(startInfo);
    ...
    table.className = 'list-table';
    const thead = document.createElement('thead');
    thead.innerHTML = '<tr><th>Topf / Bereich</th><th>Betrag</th><th>Status</th></tr>';
    table.appendChild(thead);
    const tbody = document.createElement('tbody');

    const rows = [
      { name: 'Auto', amount: ruecklagen * savingsConfig.reservePotShares['Auto'], doneLabel: 'Zurückgelegt' },
      { name: 'Urlaub', amount: ruecklagen * savingsConfig.reservePotShares['Urlaub'], doneLabel: 'Zurückgelegt' },
      { name: 'Anschaffungen (inkl. Wohnen)', amount: ruecklagen * savingsConfig.reservePotShares['Anschaffungen (inkl. Wohnen)'], doneLabel: 'Zurückgelegt' },
      { name: 'Kleidung', amount: ruecklagen * savingsConfig.reservePotShares['Kleidung'], doneLabel: 'Zurückgelegt' },
      { name: 'Freizeit', amount: ruecklagen * savingsConfig.reservePotShares['Freizeit'], doneLabel: 'Zurückgelegt' },
      { name: 'Sparen', amount: sparen, doneLabel: 'Gespart' }
    ];

    rows.forEach((entry) => {
      const tr = document.createElement('tr');
      tr.innerHTML = `<td>${entry.name}</td><td>${entry.amount.toFixed(2)} €</td><td></td>`;
      const statusCell = tr.children[2];
      const btn = document.createElement('button');
      const saved = isSavingsItemSaved(currentMonth, entry.name);
      btn.textContent = saved ? entry.doneLabel : 'Markieren';
      btn.className = saved ? 'secondary' : 'primary';
      btn.addEventListener('click', () => {
        toggleSavingsItemSaved(currentMonth, entry.name);
        saveState();
        render();
      });
  ...
  function renderPots() {
    ...
    table.appendChild(tbody);
    card.appendChild(table);
    // Zeile für die Spar-Komponente (30 % des verteilbaren Betrags) inklusive
    // manueller Ein-/Auszahlungen.
    const savingsTable = document.createElement('table');
    savingsTable.className = 'list-table';
    const sHead = document.createElement('thead');
    sHead.innerHTML = '<tr><th>Depot</th><th>Saldo</th><th>Plan (12 Monate)</th><th>Aktion</th></tr>';
    ...
    sRow.innerHTML = `<td>Sparen</td><td>${Number(state.savingsAccount.balance || 0).toFixed(2)} €</td><td>${savingsSum.toFixed(2)} €</td><td></td>`;
    const sAct = sRow.children[3];
    const sDep = document.createElement('button');
    sDep.textContent = 'Einzahlen';
    sDep.className = 'success';
    sDep.addEventListener('click', () => {
      const amountStr = prompt('Betrag zum Einzahlen in Sparen:', '0');
      if (amountStr == null) return;
      let amount = parseFloat(amountStr);
      if (isNaN(amount) || amount <= 0) {
        alert('Bitte einen positiven Betrag eingeben.');
      ...
      const desc = prompt('Beschreibung (optional):', 'Einzahlung');
      state.savingsAccount.balance = Number(state.savingsAccount.balance || 0) + amount;
      state.savingsAccount.transactions.push({ date: currentMonth, type: 'deposit', amount: amount, description: desc || '' });
      saveState();
      render();
    });
    const sWit = document.createElement('button');
    sWit.textContent = 'Ausgeben';
    sWit.className = 'danger';
    sWit.addEventListener('click', () => {
      const amountStr = prompt('Betrag zum Ausgeben aus Sparen:', '0');
      if (amountStr == null) return;
      let amount = parseFloat(amountStr);
      if (isNaN(amount) || amount <= 0) {
        alert('Bitte einen positiven Betrag eingeben.');
      ...
      const currentBalance = Number(state.savingsAccount.balance || 0);
      if (amount > currentBalance) {
        if (!confirm('Der Betrag ist größer als der Saldo. Trotzdem fortfahren?')) return;
      }
      const desc = prompt('Beschreibung (optional):', 'Ausgabe');
      state.savingsAccount.balance = currentBalance - amount;
      state.savingsAccount.transactions.push({ date: currentMonth, type: 'withdraw', amount: -amount, description: desc || '' });
      saveState();
      render();
    });
    sAct.appendChild(sDep);
    sAct.appendChild(sWit);
    ...
    const savingsManual = Number(state.savingsAccount.balance || 0);
    const totalsP = document.createElement('p');
    totalsP.innerHTML = `<strong>Gesamtsumme Rücklagen manuell:</strong> ${totalManual.toFixed(2)} € &nbsp; | &nbsp; <strong>Gesamt Rücklagen‑Plan:</strong> ${totalAuto.toFixed(2)} € &nbsp; | &nbsp; <strong>Sparen manuell:</strong> ${savingsManual.toFixed(2)} € &nbsp; | &nbsp; <strong>Sparen‑Plan:</strong> ${savingsSum.toFixed(2)} €`;
    ...
    if (selectedPotId) {
      ...
      if (isSavingsPot || pot) {
        ...
        monthList.forEach((m) => {
          ...
          let dep = 0;
          let wit = 0;
          const txSource = isSavingsPot ? (state.savingsAccount.transactions || []) : (pot.transactions || []);
          txSource.forEach((t) => {
            if (t.date === m.key) {
        ...
        const manualBalance = isSavingsPot ? Number(state.savingsAccount.balance || 0).toFixed(2) : Number(pot.balance || 0).toFixed(2);
        summaryDetail.innerHTML = `<strong>Plan-Gesamt für 12 Monate:</strong> ${sumAuto.toFixed(2)} € &nbsp; | &nbsp; <strong>Manueller Saldo:</strong> ${manualBalance} €`;
        ...
        try {
          ...
          if (data.persons && data.commonCosts && data.personalCosts && data.debts) {
            ...
            if (state.reservesSavedMonths.length && !state.reservesSavedEntries.length) {
              const items = ['Auto', 'Urlaub', 'Anschaffungen (inkl. Wohnen)', 'Kleidung', 'Freizeit', 'Sparen'];
              state.reservesSavedMonths.forEach((month) => {
                items.forEach((item) => state.reservesSavedEntries.push(`${month}|${item}`));
              });
            }
            ensureDefaultReservePots();
            saveState();
            render();
