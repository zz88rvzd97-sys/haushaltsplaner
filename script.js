/*
 * Haushaltsplaner Developer Beta 0.59
 *
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV059', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    localStorage.setItem('budgetStateV059', JSON.stringify(state));
  }
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.59 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.59 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.59 geladen';
    }
