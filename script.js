/*
 * Haushaltsplaner Developer Beta 0.48
 *
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV048', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    localStorage.setItem('budgetStateV048', JSON.stringify(state));
  }
  ...
  function renderSave() {
    ...
    h2.textContent = 'Backup & Wiederherstellen';
    card.appendChild(h2);

    const intro = document.createElement('p');
    intro.textContent = 'Für ein iCloud-Backup wird eine JSON-Datei erstellt. Auf dem iPhone öffnet sich danach die Teilen-Funktion oder der Download. Wähle dort am besten „In Dateien sichern“ und speichere das Backup in iCloud Drive.';
    ...
    importTitle.textContent = 'Backup wiederherstellen';
    importCard.appendChild(importTitle);

    const importInfo = document.createElement('p');
    importInfo.textContent = 'Wähle hier eine zuvor gesicherte Backup-Datei aus Dateien oder iCloud Drive aus. Beim Import wird der aktuelle Stand überschrieben.';
    importCard.appendChild(importInfo);

    const importRow = document.createElement('div');
    importRow.className = 'row';
    const importLabelWrap = document.createElement('div');
    ...
    importLabelWrap.appendChild(importLabel);
    importLabelWrap.appendChild(importInput);
    importRow.appendChild(importLabelWrap);
    importCard.appendChild(importRow);

    card.appendChild(importCard);
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.48 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.48 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.48 geladen';
    }
