/*
 * Haushaltsplaner Developer Beta 0.71
 *
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV071', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    localStorage.setItem('budgetStateV071', JSON.stringify(state));
  }
...
function showPersonalEditor(personId, editPost) {
  ...
  function renderSettings() {
    ...
    state.persons.forEach((p) => {
      ...
      summary.innerHTML = `<strong>Basis-Netto:</strong> ${Number(p.net || 0).toFixed(2)} € · <strong>Aktiv in ${formatMonthLabel(currentMonth)}:</strong> ${getPersonNet(p, currentMonth).toFixed(2)} € <span class="muted">(${getPersonNetSourceLabel(p, currentMonth)})</span>`;
      titleWrap.appendChild(summary);

      const shiftInfo = document.createElement('p');
      shiftInfo.className = 'small muted';
      shiftInfo.textContent = `Verschiebung: ${Number(p.shift || 0).toFixed(2)} €`;
...
function renderSavings() {
    ...
    if (beforeStart) {
      ...
      info.innerHTML = `<strong>Freier Betrag vor Puffer:</strong> ${free.toFixed(2)} € · <strong>Mindestpuffer:</strong> ${savingsConfig.minFree.toFixed(2)} € · <strong>Davon als Puffer-Ausgaben erfasst:</strong> ${bufferSpent.toFixed(2)} € · <strong>Rest-Puffer:</strong> ${remainingBuffer.toFixed(2)} € · <strong>Verteilbar für Rücklagen & Sparen:</strong> ${verteilbar.toFixed(2)} €`;
    }
    card.appendChild(info);
...
function renderPots() {
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.71 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.71 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.71 geladen';
    }
