/*
 * Haushaltsplaner Developer Beta 0.46
 *
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV046', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    localStorage.setItem('budgetStateV046', JSON.stringify(state));
  }
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.46 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.46 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.46 geladen';
    }
