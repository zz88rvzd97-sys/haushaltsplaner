/*
 * Haushaltsplaner Developer Beta 0.69
 *
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV069', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    localStorage.setItem('budgetStateV069', JSON.stringify(state));
  }
...
2 = ab diesem Monat dauerhaft`,
  ...
  function renderOverview() {
    ...
    info.textContent = 'Netto-Anpassungen öffnest du hier jetzt direkt über „Bearbeiten“.';
...
function renderPots() {
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.69 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.69 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.69 geladen';
    }
