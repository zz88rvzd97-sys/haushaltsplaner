/*
 * Haushaltsplaner Developer Beta 0.65
 *
 * Diese Version ergänzt für gemeinsame und persönliche Posten eine
...
(() => {
    ...
    if (!saved) {
      ...
      for (const k of fallback) {
        ...
        if (data) {
          ...
          localStorage.setItem('budgetStateV065', data);
          break;
        }
      }
    }
  ...
  function saveState() {
    localStorage.setItem('budgetStateV065', JSON.stringify(state));
  }
...
function renderPots() {
  ...
  if (versionChip) {
    ...
    if (params.has('refresh')) {
      versionChip.textContent = 'Update 0.64 geladen';
      setTimeout(() => {
        versionChip.textContent = 'Version 0.64 geladen';
      }, 2500);
    } else {
      versionChip.textContent = 'Version 0.64 geladen';
    }
